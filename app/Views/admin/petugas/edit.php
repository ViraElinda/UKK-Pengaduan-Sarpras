...
<div class="min-h-screen bg-ui-page py-8 px-4 sm:px-6 lg:px-8">
  <div class="max-w-3xl mx-auto">
    <div class="mb-6">
      ...
      <p class="text-gray-600">Perbarui data petugas.</p>
    </div>

    <div class="bg-white/90 rounded-xl shadow p-6">
      <form action="<?= base_url('admin/petugas/update/'.$petugas['id_petugas']); ?>" method="post" class="space-y-4">
        ...
        <div>
          ...
          <select name="id_user" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200">
            ...
            <?php if (!empty($users)): foreach($users as $u): ?>
              <option value="<?= esc($u['id_user']) ?>" <?= ($petugas['id_user'] == $u['id_user']) ? 'selected' : '' ?>><?= esc($u['username']) ?> — <?= esc($u['nama_pengguna'] ?? '') ?></option>
        ...
        <div>
          ...
          <input type="text" name="nama" value="<?= esc($petugas['nama']) ?>" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" required />
        </div>
