...
<nav class="bg-gradient-to-r from-indigo-600 via-blue-600 to-blue-500 shadow-lg sticky top-0 z-50 backdrop-blur-sm border-b border-white/10">
  ...
  <a href="<?= base_url('user/riwayat') ?>" class="nav-link flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-200 <?= ($currentUri == 'user/riwayat') ? 'bg-white/20 text-white shadow-md' : 'text-white/90 hover:bg-white/10 hover:text-white' ?>">
        ...
        <button id="themeToggle" title="Toggle theme" class="p-2 rounded-full hover:bg-white/10 transition mr-2 text-white" aria-pressed="false">
          <svg id="themeIcon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><!-- sun by default -->
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v2m0 14v2m9-9h-2M5 12H3m15.364-6.364l-1.414 1.414M7.05 16.95l-1.414 1.414M16.95 16.95l1.414 1.414M6.343 6.343L4.929 4.929"/>
          </svg>
        </button>
...
<!-- Theme toggle script: toggles html.dark-mode and persists choice in localStorage -->
<script>
  (function(){
    ...
    function setIcon(isDark){
      ...
      if(isDark){
        ...
        // sun icon
        icon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v2m0 14v2m9-9h-2M5 12H3m15.364-6.364l-1.414 1.414M7.05 16.95l-1.414 1.414M16.95 16.95l1.414 1.414M6.343 6.343L4.929 4.929"/>';
      }
    }
